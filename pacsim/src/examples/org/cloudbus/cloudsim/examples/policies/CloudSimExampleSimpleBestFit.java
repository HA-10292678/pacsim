...
import org.cloudbus.cloudsim.pam.PAMVmAllocationPolicySimpleBestFit;
...
public class CloudSimExampleSimpleBestFit {
	
public static void main(String args[]){
	Log.printLine("Starting CloudSimExampleSimpleBestFit...");
	...
	try {
		...
		CloudSimExampleSimpleBestFit csps=new CloudSimExampleSimpleBestFit();
		PAMDatacenter datacenter0 = csps.createDatacenter("Datacenter_0");
		csps.createDCManager(datacenter0);

		@SuppressWarnings("unused")
		PAMDatacenterBrokerExtended pamDBE=new PAMDatacenterBrokerExtended("PAMDatacenterBrokerExtended","workloadpattern_103");
		
		CloudSim.startSimulation();
		CloudSim.stopSimulation();
		datacenter0.printDebts();

		Log.printLine("CloudSimExampleSimpleBestFit finished!");
	}
	catch (Exception e) {
		e.printStackTrace();
...
private  PAMDatacenter createDatacenter(String name) {
 ...
 // need to store Pe id and MIPS Rating
	...
	try {
		datacenter = new PAMDatacenter(name, characteristics, new PAMVmAllocationPolicySimpleBestFit(hostList), storageList, 0);
	} catch (Exception e) {
		e.printStackTrace();
	}
