package org.cloudbus.cloudsim.examples.policies;
...
import org.cloudbus.cloudsim.pam.PAMVmAllocationPolicyPowerState;
import org.cloudbus.cloudsim.pam.PAMVmSchedulerSpaceShared;
...
public class CloudSimExampleBestFitDecreasing {
	
public static void main(String args[]){
	Log.printLine("Starting CloudSimExampleBestFitDecreasing...");
	...
	try {
		...
		CloudSimExampleBestFitDecreasing csps=new CloudSimExampleBestFitDecreasing();
		PAMDatacenter datacenter0 = csps.createDatacenter("Datacenter_0");
		csps.createDCManager(datacenter0);

		@SuppressWarnings("unused")
		PAMDatacenterBrokerExtended pamDBE=new PAMDatacenterBrokerExtended("PAMDatacenterBrokerExtended","DynamicWorkload_5");
		
		CloudSim.startSimulation();
		CloudSim.stopSimulation();
		datacenter0.printDebts();

		Log.printLine("CloudSimExampleBestFitDecreasing finished!");
	}
	catch (Exception e) {
		e.printStackTrace();
 ...
 // need to store Pe id and MIPS Rating
	...
	hostList.add(
		new PAMHost(
			...
			storage,
			chipList,
			new PAMVmSchedulerSpaceShared(chipList),
			hostState
		)
	);
	} // This is our machine
	...
	try {
		datacenter = new PAMDatacenter(name, characteristics, new PAMVmAllocationPolicyPowerState(hostList), storageList, 0);
	} catch (Exception e) {
		e.printStackTrace();
	}
