package org.cloudbus.cloudsim.example.chipawarepolicies;
...
import org.cloudbus.cloudsim.pam.PAMVmSchedulerChipAware;
import org.cloudbus.cloudsim.pamchipawarepolicies.PAMVmAllocationPolicyChipAwareSimpleBestFit;
...
public class CloudSimChipAwareSimpleBestFit {
	
public static void main(String args[]){
	Log.printLine("Starting CloudSimChipAwareSimpleBestFit...");
	...
	try {
		...
		CloudSimChipAwareSimpleBestFit csps=new CloudSimChipAwareSimpleBestFit();
		PAMDatacenter datacenter0 = csps.createDatacenter("Datacenter_0");
		csps.createDCManager(datacenter0);

		@SuppressWarnings("unused")
		PAMDatacenterBrokerExtended pamDBE=new PAMDatacenterBrokerExtended("PAMDatacenterBrokerExtended","workloadpattern_3");
		
		CloudSim.startSimulation();
		CloudSim.stopSimulation();
		datacenter0.printDebts();

		Log.printLine("CloudSimChipAwareSimpleBestFit finished!");
	}
	catch (Exception e) {
		e.printStackTrace();
 ...
 // need to store Pe id and MIPS Rating
	...
	hostList.add(
		new PAMHost(
			...
			storage,
			chipList,
			new PAMVmSchedulerChipAware(chipList),
			hostState
		)
	);
	} // This is our machine
	...
	try {
		datacenter = new PAMDatacenter(name, characteristics, new PAMVmAllocationPolicyChipAwareSimpleBestFit(hostList), storageList, 0);
	} catch (Exception e) {
		e.printStackTrace();
	}
