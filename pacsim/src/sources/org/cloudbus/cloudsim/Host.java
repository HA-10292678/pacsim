...
//			}
	...
	public boolean vmCreate(Vm vm) {
		...
		//changed
		for(int j=0;j<vm.getPesNumber();j++){
			for(int i=0;i<this.getPeList().size();i++){
				if(getPeList().get(i).getStatus()==FREE){
					getPeList().get(i).setStatus(BUSY);
					break;
				}
			}
		}
	...
	public void vmDestroy(Vm vm) {
		...
		for(int j=0;j<vm.getPesNumber();j++){
			for(int i=0;i<getPeList().size();i++){
				if(getPeList().get(i).getStatus()==BUSY){
					getPeList().get(i).setStatusFree();
					break;
				}
			}
		}
