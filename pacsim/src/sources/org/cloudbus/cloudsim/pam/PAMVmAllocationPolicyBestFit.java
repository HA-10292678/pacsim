...
public class PAMVmAllocationPolicyBestFit extends PAMVmAllocationPolicy {
	 ...
	 * @post $none
	 */
	public PAMVmAllocationPolicyBestFit(List<? extends PAMHost> list) {
		super(list);
				...
				while( it. hasNext() ){
					...
					//	hostToPesInNapMap.put(host.getId(), pesInNap);
						...
						if(pesInNap>=requiredPes){
							...
							System.out.println(CloudSim.getClockTick()+": Pes in Nap is "+pesInNap+" in Host #"+idx+" for Vm #"+vm.getId()+" with required Pes: "+vm.getPesNumber());
							System.out.println(CloudSim.getClockTick()+": SLA Violating... Moving "+remainingPes+" Pes in Host #"+idx+" from WRINKLE to NAP...");
