...
public class PAMVmAllocationPolicySimpleWorstFit extends PAMVmAllocationPolicy {
	 ...
	 * @post $none
	 */
	public PAMVmAllocationPolicySimpleWorstFit(List<? extends PAMHost> list) {
		super(list);
	...
	public boolean allocateHostForVm(PAMVm vm) {
		...
		if (!getVmTable().containsKey(vm.getUid())) {
			do{
				...
				if(lessInNap>=requiredPes){
					result = host.vmCreate(vm);
				
				}else{
					remainingPes=requiredPes-lessInNap;
					pesInWinkle=ChipList.getPesInWinkle(host.getChipList());
					if(pesInWinkle>=remainingPes){
						incVmRequestNotFullfilled();
						System.out.println(CloudSim.getClockTick()+": Pes in Nap is "+lessInNap+" in Host #"+idx+" for Vm #"+vm.getId()+" with required Pes: "+vm.getPesNumber());
						...
						hostToPesInNapMap.put(host.getId(), Integer.MAX_VALUE);
					}
				}
