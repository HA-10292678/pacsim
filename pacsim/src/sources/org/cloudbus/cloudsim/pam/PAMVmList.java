...
public class PAMVmList {
	 ...
	 * @post $none
	 */
	public static <T extends PAMVm> T getById(List<T> vmList, int id) {
		for (T vm : vmList) {
			if (vm.getId() == id) {
				return vm;
			}
	 ...
	 * @post $none
	 */
	public static <T extends PAMVm> T getByIdAndUserId(List<T> vmList, int id, int userId) {
		for (T vm : vmList) {
