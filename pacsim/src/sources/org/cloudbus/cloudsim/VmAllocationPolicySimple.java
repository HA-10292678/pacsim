...
public class VmAllocationPolicySimple extends VmAllocationPolicy {
		...
		if (!getVmTable().containsKey(vm.getUid())) { //if this vm was not created
			do {//we still trying until we find a host or until we try all of them
				...
				if (result) { //if vm were succesfully created in the host
					...
					/*for(int j=0;j<requiredPes;j++){
						for(int i=0;i<host.getPeList().size();i++){
							if(host.getPeList().get(i).getStatus()==FREE){
								host.getPeList().get(i).setStatusBusy();
								break;
							}
						}
					}*/
	...
	public void deallocateHostForVm(Vm vm) {
		...
		/*for(int j=0;j<pes;j++){
			for(int i=0;i<host.getPeList().size();i++){
				if(host.getPeList().get(i).getStatus()==BUSY){
					host.getPeList().get(i).setStatusFree();
					break;
				}
			}
		}*/
